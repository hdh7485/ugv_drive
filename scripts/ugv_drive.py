...
def twist_callback(twist_callback_data):
    # rospy.loginfo(twist_callback_data)
    steer_angle = [0.0, 0.0, 0.0, 0.0]
    steer_speed = [0.0, 0.0, 0.0, 0.0]
    wheel_speed = [0.0, 0.0, 0.0, 0.0]
    steer_angle, steer_speed, wheel_speed = calculate_joint_kinetics(twist_callback_data)
...
def atan4quad(y, x):
    ...
    # result = normalizePi(result)
    return result

def calculate_joint_kinetics(twist_data):
    wheel_position = [{'x':0.6, 'y':0.4}, {'x':0.6, 'y':-0.4}, {'x':-0.6, 'y':-0.4}, {'x':-0.6, 'y':0.4}]
    # wheel_radius = 0.0254 / 2
    wheel_radius = 0.0127
    steer_angle = [0.0, 0.0, 0.0, 0.0]
    steer_speed = [0.0, 0.0, 0.0, 0.0]
    wheel_speed = [0.0, 0.0, 0.0, 0.0]
